...
module Monster {
    export class DanceDanceRevolutionArrow implements eg.IUpdateable, eg.IDisposable {
        ...
        private Rect: eg.Graphics.Rectangle;     
        private Rate: number;
        public Direction: ArrowDirection;
        public IsFinished: boolean;   
        public WasScored: boolean;
        ...
        public Finish(wasScored: boolean = false, good: boolean = false): void {
    ...
    export class ItsBasicallyDanceDanceRevolution implements eg.IUpdateable, eg.IDisposable {
        ...
        private ScoreBackground: eg.Graphics.Rectangle;
        private ScoreLabel: eg.Graphics.Text2d;
        private ScoreText: eg.Graphics.Text2d;
        private ScoreMultiplierLabel: eg.Graphics.Text2d;
        private ScoreMultiplierText: eg.Graphics.Text2d;

        private Arrows: Array<DanceDanceRevolutionArrow>;
        private HandledArrows: Array<DanceDanceRevolutionArrow>;
        private MusicStart: number;
        ...
        constructor(screen: eg.Rendering.Scene2d, content: eg.Content.ContentManager, tweenManager: Monster.TweenManager, keyboardHandler: eg.Input.KeyboardHandler, music: eg.Sound.AudioClip) {
            ...
            this.HandledArrows = new Array<DanceDanceRevolutionArrow>();

            this.KeyDown = (event: eg.Input.KeyboardCommandEvent) => {
                if (event.Key === "up" || event.Key === "w") {
                    this.HandleKeypress(ArrowDirection.Up);
                }
                else if (event.Key === "down" || event.Key === "s") {
                    this.HandleKeypress(ArrowDirection.Down);
                }
                else if (event.Key === "left" || event.Key === "a") {
                    this.HandleKeypress(ArrowDirection.Left);
                }
                else if (event.Key === "right" || event.Key === "d") {
                    this.HandleKeypress(ArrowDirection.Right);
                }
                else if (event.Key === "q") {
                    console.log(Date.now().valueOf() - this.MusicStart);
                }
            };
        ...
        public Update(gameTime: eg.GameTime): void {
            ...
            switch (self.State) {            
                ...
                case 1:
                    ...
                    self.HandledArrows.forEach((arrow) => { arrow.Update(gameTime); });
                    self.HandledArrows = _.reject(self.HandledArrows, (arrow) => {
                        return arrow.IsFinished;
                    });
                    break;
            };
        ...
        private HandleKeypress(direction: ArrowDirection) {
            var self = this;
            ...
            if (topArrow.Direction === direction) {                
                ...
                if (baseScore > 0) {
                    ...
                    if (this.LongestChain < this.Chain) this.LongestChain = this.Chain;
